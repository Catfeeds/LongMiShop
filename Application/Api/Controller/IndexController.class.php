...
class IndexController extends BaseController {
    ...
    public function home(){
        //获取轮播图
        $data = M('ad')->where('pid = 2')->field(array('ad_link','ad_name','ad_code'))->cache(true,LONGMI_CACHE_TIME)->select();
        //广告地址转换
        foreach($data as $k=>$v){
//            exit($this->http_url);
...
//                exit($this->http_url);
        ...
        //获取大分类
        $category_arr = M('goods_category')->where('parent_id=0')->field('id,name')->limit(3)->cache(true,LONGMI_CACHE_TIME)->select();
        $result = array();
        foreach($category_arr as $c){
            ...
            //$goods = M()->query($sql);
            $goodsList = M('goods')->where("1=1")->limit(4)->cache(true,LONGMI_CACHE_TIME)->getField("goods_id,goods_name,original_img,shop_price");
