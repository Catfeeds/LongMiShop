...
//        'index:index'=>array('{:module}_{:controller}_{:action}',MY_CACHE_TIME),  // 首页静态缓存 3秒钟
        //'index:goodsList'=>array('{:module}_{:controller}_{:action}',300),  // 列表页静态缓存 3秒钟 无参数 post 提交的很难缓存
        //'Goods:goodsList'=>array('{:module}_{:controller}_{:action}_{id})_{p}',MY_CACHE_TIME),  // 列表页静态缓存 3秒钟
...
//        'Goods:ajax_consult'=>array('{:module}_{:controller}_{:action}_{goods_id}_{consult_type}_{p}',MY_CACHE_TIME),  // 商品咨询页静态缓存 3秒钟
        // 商品详情的规格价格缓存在 ajaxGoodsPrice 函数里面 S($keys,$price,300);// 缓存数据300秒
//        'channel:index'=>array('{:module}_{:controller}_{:action}_{id}',MY_CACHE_TIME),  // 列表页静态缓存 3秒钟
    ),
    //默认错误跳转对应的模板文件
